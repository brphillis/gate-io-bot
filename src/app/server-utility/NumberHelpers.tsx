...
export const decimalsStartWithMoreThanThreeZeros = (
  numberString: string
): boolean => {
  // Extract the decimal part from the number string
  const decimalPart = numberString.split(".")[1] || "";

  // Define the regular expression pattern
  const pattern = /^0{4,}/;

  // Test if the decimal part matches the pattern
  return pattern.test(decimalPart);
};
